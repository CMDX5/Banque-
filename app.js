...
function renderVirtualCard(data) {
  ...
  if (!data) {
    virtualCardBox.innerHTML = '<p class="muted small">Aucune carte virtuelle créée pour l’instant.</p>';
    return;
  }
...
async function generateVirtualCardFor(uid) {
  try {
    ...
    console.error(err);
    if (virtualCardMessage) {
      showMessage(virtualCardMessage, formatError(err, 'Erreur lors de la génération de la carte.'), 'error');
    }
  }
}
...
async function toggleVirtualCardStatus(uid) {
  try {
    ...
    if (!snap.exists) {
      if (virtualCardMessage) {
        showMessage(virtualCardMessage, 'Aucune carte virtuelle à modifier.', 'error');
      }
      return;
    }
    ...
    await loadVirtualCard(uid);
    if (virtualCardMessage) {
      showMessage(virtualCardMessage, newStatus === 'frozen' ? 'Carte virtuelle gelée.' : 'Carte virtuelle réactivée.', 'success');
    }
  } catch (err) {
    console.error(err);
    if (virtualCardMessage) {
